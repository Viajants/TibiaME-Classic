...
import com.pupilla.dpk.Screens.TasksScreen;
...
public class Collision implements ContactListener {
    ...
    public void beginContact(Contact contact) {
        ...
        // collision with Hause
        if(fixA.getUserData() instanceof Portal || fixB.getUserData() instanceof Hause){
            Fixture player = fixA.getUserData() == "player" ? fixA : fixB;
            Fixture hause = player==fixA ? fixB : fixA;
            if(hause.getUserData() instanceof Hause && player.getUserData() == "player"){
                Gdx.app.debug(TAG, "Collision with Hause");
                Hause h = (Hause) hause.getUserData();

             //   PlayScreen.player.map = h.destinationMap;
