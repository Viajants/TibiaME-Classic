...
import com.pupilla.dpk.Backend.Hause;
...
public class PlayScreen extends ApplicationAdapter implements Screen {
    ...
    public static ArrayList <Hause> hauses =new ArrayList<Hause>();
    public PlayScreen(Game game, boolean newGame){
        ...
        camera = new OrthographicCamera();
            mapManager = new MapManager(MapConstants.AUREA, world);
        ...
        if(newGame){
            ...
            player.setup();
                ...
                //Mines
            player.defineBody(new Vector2(71*Constants.UNIT_SCALE+14, 150*Constants.UNIT_SCALE+14));


            //player.defineBody(new Vector2(122*Constants.UNIT_SCALE+14, 125*Constants.UNIT_SCALE+14));
        //    player.defineBody(new Vector2(137*Constants.UNIT_SCALE, 89*Constants.UNIT_SCALE));
            //Demon Placeplayer.defineBody(new Vector2(96 * Constants.UNIT_SCALE + 13, 110 * Constants.UNIT_SCALE + 14));
           // player.defineBody(new Vector2(114*Constants.UNIT_SCALE, 89*Constants.UNIT_SCALE));
         //  player.defineBody(new Vector2(171*Constants.UNIT_SCALE, 60*Constants.UNIT_SCALE));

                //Orc Vilage
        //   player.defineBody(new Vector2(158*Constants.UNIT_SCALE, 261*Constants.UNIT_SCALE));


            //ORC DESERT CAVE
          //  player.defineBody(new Vector2(179*Constants.UNIT_SCALE, 256*Constants.UNIT_SCALE));
    ...
    public void render(float delta) {
        ...
        renderer.render();
        //DRAW MAP COLLISION POLYGONSHAPE LINES
        b2dr.AABB_COLOR.add(Color.YELLOW);
        ...
        if(hud.isTouched){
            switch(hud.direction){
                case LEFT:
                    ...
                    player.b2body.setLinearVelocity(-(100000* Gdx.graphics.getDeltaTime()), player.b2body.getLinearVelocity().y);
                    player.currentSprite.setPosition(player.b2body.getPosition().x, player.b2body.getPosition().y);
                    player.alive = true;
 ...
 //        I Use my own stair image
    ...
    private void createHauseBodies(){
        for(int i=0; i<hauses.size(); i++){
            //creating hause body
          //hauses.get(i).createBody(world);
        }
    }
